...
```http://localhost:8181/examples/get_token.php``` для получения Access Token. ```CLIENT_REDIRECT_URI=``` будет содержать адрес файла для генерации токена указанный при создании интеграции, пример ```CLIENT_REDIRECT_URI="[домен сайта]/examples/get_token.php"```
